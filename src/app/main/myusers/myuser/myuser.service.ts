...
@Injectable()
export class MyUserService implements Resolve<any>
{
    routeParams: any;
    user: User;
    ...
    getUser(): Promise<any> {
        return new Promise((resolve, reject) => {
            if (this.routeParams.id === 'new') {
                ...
                this._httpClient.get(Config.prop.apiEndpoint + "myusers/search.php?oid=" + this.routeParams.id)
    ...
    saveUser(user, changePassword: boolean, logo: File): Promise<any> {
        ...
        return new Promise((resolve, reject) => {
            this._httpClient.post(Config.prop.apiEndpoint + "myusers/update.php", formData)
                .subscribe((response: any) => {
    ...
    addUser(user, logo: File): Promise<any> {
        ...
        return new Promise((resolve, reject) => {
            this._httpClient.post(Config.prop.apiEndpoint + "myusers/insert.php", formData)
                .subscribe((response: any) => {
                    //this.user = response[0];
                    this.user = response;
                    this.onUserChanged.next(this.user);
